...
```{r}
#Import Data
report_cards <- read_csv("wake county schools report card 2015-16.csv")
...
```{r}
#Import Data
ice <- read_csv("nenana ice breakup.csv")
...
Problem #5
```{r}
college_access <- read_csv("College Access Index.csv")
...
Problem #6
```{r}
reptile <- read_csv("REPTILE BRAIN BODY WEIGHTS.csv")
mammal <- read_csv("mammals brain body mass.csv")
...
Problem #8 <https://rpubs.com/aaronsc32/regression-confidence-prediction-intervals>
```{r}
bodyfat <- read_csv("bodyfat.csv")

bodyfat_modified <- 
  bodyfat %>%
  rename(waist = `Waist (in.)`,
         weight = `Weight (lb)`,
         body_fat = `Body Fat (%)`)
bodyfat_modified
...
Problem #10
```{r}
teams <- read_csv("top 50 metro area by pop inc pop percapinc profranch.csv")
...
Problem #12
```{r}
blood <- read_csv("blood test human age.csv")

blood_mod <-
  blood %>%
  rename(blood_test = `Blood Test Measure`,
         age = `Age in Years`)
blood_mod
